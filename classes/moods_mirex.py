...
class MoodsMirexClassifier:
    """
    Classifies the mood of an audio file into 5 MIREX mood clusters.
    """
    ...
    def __init__(self):
        ...
        - moods_mirex-audioset-vggish-1.pb (classification model)
        """
        # Paths to the models
        self.embedding_model_path = os.path.join(MODELS, "audioset-vggish-3.pb")
        self.classification_model_path = os.path.join(MODELS, "moods_mirex-audioset-vggish-1.pb")
    ...
    def _initialize_models(self):
        ...
        # Initialize the MIREX mood classification model
    ...
    def classify_moods(self, audio_file):
        """
        Classifies the mood of the given audio file.

        Returns:
            dict: A dictionary with MIREX mood clusters and their probabilities.
        """
        ...
        mood_scores = {clusters[i + 1]: float(predictions[i]) for i in range(len(predictions))}
...
def main():
    ...
    try:
        # Classify moods
        mood_scores = classifier.classify_moods(audio_file)
        print("MIREX Mood Scores:")
