...
class VoiceGenderClassifier:
    """
    Classifies the singing voice gender of an audio file into `female` or `male`.
    """
    ...
    def __init__(self):
        ...
        - gender-audioset-vggish-1.pb (classification model)
        """
        # Paths to the models
        self.embedding_model_path = os.path.join(MODELS, "audioset-vggish-3.pb")
        self.classification_model_path = os.path.join(MODELS, "gender-audioset-vggish-1.pb")
    ...
    def classify_gender(self, audio_file):
        """
        Classifies the singing voice gender of the given audio file.

        Returns:
            dict: A dictionary with probabilities for `female` and `male`.
        """
        ...
        gender_scores = {classes[i]: float(predictions[i]) for i in range(len(classes))}
...
def main():
    ...
    try:
        ...
        gender_scores = classifier.classify_gender(audio_file)
        print("Voice Gender Scores:")
        for label, score in gender_scores.items():
